...
interface RevenusPro {
  salaire: string
  aPrimes: boolean; primesAnnuelles: string; primesPeriode: string
  aAvantages: boolean; avantagesTypes: string[]; avantageValeurs: Record<string,string>
  remunNette: string; dividendesSociete: string; cotisationsSociales: string
  aRemplacement: boolean; remplacementType: string; remplacementMontant: string
}
...
interface Bloc4State {
  ...
  dividendesCoupons: string; interetsLivrets: string
  aPlusValues: boolean; pvTypes: string[]; pvMontant: string
  aAutresRevenus: boolean; autresRevenusMontants: Record<string,string>
  // Dépenses
...
const DEPENSES_DEFAUT: Omit<DepenseMensuelle,'montant'|'pct'>[] = [
  { id:'loyer',      label:'Loyer / Charges habitation', prefilled:false, mode:'euro', emoji:'🏠' },
  { id:'transport',  label:'Transports',                 prefilled:false, mode:'euro', emoji:'🚗' },
  { id:'nourriture', label:'Alimentation',               prefilled:false, mode:'pct',  emoji:'🛒' },
  { id:'invest',     label:'Investissement / Épargne',   prefilled:false, mode:'pct',  emoji:'📈' },
  { id:'loisirs',    label:'Loisirs & sorties',          prefilled:false, mode:'pct',  emoji:'🎭' },
  { id:'sante',      label:'Santé & assurances',         prefilled:false, mode:'euro', emoji:'🏥' },
  { id:'abos',       label:'Abonnements',                prefilled:false, mode:'euro', emoji:'📱' },
  { id:'autres',     label:'Autres dépenses',            prefilled:false, mode:'pct',  emoji:'💳' },
]
...
function calcTMI(revImp:number, nbParts:number) {
  ...
  for(let i=0;i<tr.length;i++) if(qi<=tr[i].s) return {tmi:tr[i].t, depuisMontant:i>0?tr[i-1].s*nbParts:0, resteAvant:i<tr.length-1?(tr[i].s-qi)*nbParts:0}
  return {tmi:45,depuisMontant:0,resteAvant:0}
}
...
export default function Bloc4() {
  ...
  const foyerBloc1 = loadLS<{typeLogement?:string;enfantsCharge?:number}>('patrisim_bloc1_foyer',{})
  const nbEnfants = foyerBloc1.enfantsCharge||0

  // Pré-remplissage mensualités Bloc3
  const bloc3 = loadLS<{creditsImmo?:{mensualiteHA?:string;mensualiteAssurance?:string}[];creditsConso?:{mensualite?:string}[]}>('patrisim_bloc3',{})
  const mensualitesBloc3 = (bloc3.creditsImmo||[]).reduce((a,c)=>a+pn(c.mensualiteHA)+pn(c.mensualiteAssurance),0)+(bloc3.creditsConso||[]).reduce((a,c)=>a+pn(c.mensualite),0)

  // Pré-remplissage revenus Bloc2
  const bloc2 = loadLS<{biens?:{loue?:boolean;location?:{loyerMensuel?:string;tauxOccupation?:string}}[];avs?:{valeurRachat?:string;rendement?:string}[];peas?:{valeur?:string;rendement?:string}[]}>('patrisim_bloc2',{})
  ...
  const revenusFinancBloc2 = [...(bloc2.avs||[]).map(av=>pn(av.valeurRachat||'0')*pn(av.rendement||'0')/100),...(bloc2.peas||[]).map(p=>pn(p.valeur||'0')*pn(p.rendement||'0')/100)].reduce((a,b)=>a+b,0)
  ...
  const [state, setState] = useState<Bloc4State>(() => {
    const s = loadLS<Bloc4State>('patrisim_bloc4', {
      pdfStatus:'idle', pdfFileName:'', pdfSkipped:false,
      p1Pro:defaultPro(), p2Pro:defaultPro(),
      revenusFonciersB: loyersBloc2>0?String(Math.round(loyersBloc2)):'',
      revenusFinanciers: revenusFinancBloc2>0?String(Math.round(revenusFinancBloc2)):'',
      dividendesCoupons:'', interetsLivrets:'',
      aPlusValues:false, pvTypes:[], pvMontant:'',
      aAutresRevenus:false, autresRevenusMontants:{},
      depenses: DEPENSES_DEFAUT.map(d=>({...d,montant:d.id==='loyer'&&mensualitesBloc3>0?String(Math.round(mensualitesBloc3)):'',pct:''})),
  ...
  const revenusPatrimo = pn(state.revenusFonciersB)/12 + pn(state.revenusFinanciers)/12 + pn(state.dividendesCoupons)/12
  const autresRev = Object.values(state.autresRevenusMontants).reduce((a,v)=>a+pn(v),0)
  const revenusTotaux = revP1 + revP2 + revenusPatrimo + autresRev

  // Dépenses : convertir % en €
  const depensesEnEuros = state.depenses.map(d => {
    if(d.mode==='euro') return pn(d.montant)
    if(d.mode==='pct'&&d.pct) return Math.round(revenusTotaux*pn(d.pct)/100)
    return pn(d.montant)
  })
  const totalDepenses = depensesEnEuros.reduce((a,b)=>a+b,0) + (state.aPension?pn(state.pensionMontant):0)
      ...
      <div className="max-w-4xl mx-auto px-8 py-8 pb-40">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1 max-w-xs h-1 bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-[#185FA5] rounded-full" style={{width:'57%'}}/></div>
          ...
          <p className="text-[14px] text-gray-400 mt-1">Revenus, dépenses et situation fiscale — commencez par importer votre avis d'imposition.</p>
        </div>
        ...
        {!state.pdfSkipped && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-8 space-y-4">
            <div>
              ...
              <p className="text-[12px] text-gray-400">L'IA extraira automatiquement vos données fiscales — plus rapide que de tout saisir à la main.</p>
            </div>
            ...
            {state.pdfStatus==='idle'&&(
              <div
                ...
                className="border-2 border-dashed border-[#185FA5]/40 bg-[#E6F1FB]/40 hover:bg-[#E6F1FB] hover:border-[#185FA5] rounded-2xl p-10 text-center cursor-pointer transition-all"
              >
                ...
                <p className="text-[14px] font-semibold text-[#0C447C]">Glissez votre PDF ici ou cliquez</p>
                <p className="text-[12px] text-[#185FA5] mt-1">Avis d'imposition · Format PDF · Max 10 Mo</p>
              </div>
            )}
            ...
            <button type="button" onClick={()=>upd('pdfSkipped',true)} className="text-[12px] text-gray-400 hover:text-gray-600 transition-colors">
              Passer cette étape — je saisirai manuellement →
            </button>
          </div>
        )}
        {state.pdfSkipped&&(
          <div className="mb-6">
            <button type="button" onClick={()=>upd('pdfSkipped',false)} className="text-[12px] text-[#185FA5] hover:text-[#0C447C]">← Importer un avis d'imposition</button>
          </div>
        )}
        ...
        {isCouple ? (
          <div className="grid grid-cols-2 gap-4 mb-6">
            {[{pro:state.p1Pro,setPro:(p:RevenusPro)=>upd('p1Pro',p),label:p1Label,statut:pro1Bloc1.statut},
              {pro:state.p2Pro,setPro:(p:RevenusPro)=>upd('p2Pro',p),label:p2Label,isP2:true,statut:pro2Bloc1.statut}].map(({pro,setPro,label,isP2,statut})=>(
              <div key={label} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                ...
                {isTNS(statut)?(
                  ...
                  <Field label="Salaire net mensuel" tooltip="Après prélèvement à la source, tel qu'il apparaît sur votre virement">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4 mb-6">
          ...
          <div className="space-y-3">
            {state.depenses.map((d,i)=>{
              const montantCalc = d.mode==='pct'&&d.pct ? Math.round(revenusTotaux*pn(d.pct)/100) : pn(d.montant)
              return (
                <div key={d.id} className="flex items-center gap-4 p-3 bg-gray-50 rounded-xl">
                  ...
                  <span className="text-[12px] font-medium text-gray-700 w-40 flex-shrink-0">{d.label}</span>

                  {/* Toggle € / % */}
                  <div className="flex rounded-lg overflow-hidden border border-gray-200">
                    <button type="button" onClick={()=>upd('depenses',state.depenses.map((x,j)=>j===i?{...x,mode:'euro'}:x))}
                      className={`px-2.5 py-1 text-[11px] font-medium transition-colors ${d.mode==='euro'?'bg-[#185FA5] text-white':'bg-white text-gray-400'}`}>€</button>
                    <button type="button" onClick={()=>upd('depenses',state.depenses.map((x,j)=>j===i?{...x,mode:'pct'}:x))}
                      className={`px-2.5 py-1 text-[11px] font-medium transition-colors ${d.mode==='pct'?'bg-[#185FA5] text-white':'bg-white text-gray-400'}`}>%</button>
                  </div>

                  {/* Input */}
                  <div className="w-32">
                    {d.mode==='euro'?(
                      <Input value={d.montant} onChange={v=>upd('depenses',state.depenses.map((x,j)=>j===i?{...x,montant:v}:x))} placeholder="0" suffix="€/mois"/>
                    ):(
                      <div className="space-y-1">
                        <Input value={d.pct} onChange={v=>upd('depenses',state.depenses.map((x,j)=>j===i?{...x,pct:v}:x))} placeholder="0" suffix="%"/>
                        {d.pct&&<p className="text-[10px] text-gray-400 text-right">≈ {fmt(montantCalc)} €</p>}
                      </div>
                    )}
                  </div>
                  ...
                  {revenusTotaux>0&&(
                    <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div className="h-full bg-[#185FA5] rounded-full transition-all"
                        style={{width:`${Math.min(100, montantCalc/revenusTotaux*100)}%`}}/>
                    </div>
                  )}
                  {revenusTotaux>0&&<span className="text-[11px] text-gray-400 w-8 text-right">{Math.round(montantCalc/revenusTotaux*100)}%</span>}
                </div>
              )
            })}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Revenu imposable"><Input value={state.fiscal.revenuImposable} onChange={v=>upd('fiscal',{...state.fiscal,revenuImposable:v})} placeholder="40 000" suffix="€"/></Field>
            <Field label="Impôt sur le revenu net"><Input value={state.fiscal.impotNet} onChange={v=>upd('fiscal',{...state.fiscal,impotNet:v})} placeholder="0" suffix="€"/></Field>
            <Field label="Prélèvements sociaux"><Input value={state.fiscal.prelevementsSociaux} onChange={v=>upd('fiscal',{...state.fiscal,prelevementsSociaux:v})} placeholder="0" suffix="€"/></Field>
          </div>

          {/* Calculs auto */}
          {rfr>0&&(
            <div className="grid grid-cols-3 gap-3 mt-2">
              <div className="bg-gray-50 rounded-xl p-3 text-center"><p className="text-[10px] text-gray-400 uppercase mb-1">Taux moyen</p><p className="text-[18px] font-bold text-[#185FA5]">{tauxMoyen}%</p></div>
              <div className="bg-gray-50 rounded-xl p-3 text-center"><p className="text-[10px] text-gray-400 uppercase mb-1">TMI estimé</p><p className="text-[18px] font-bold text-gray-800">{tmi}%</p></div>
              <div className="bg-gray-50 rounded-xl p-3 text-center"><p className="text-[10px] text-gray-400 uppercase mb-1">Pression fiscale</p><p className="text-[16px] font-bold text-gray-800">{fmt(ir+ps)} €/an</p></div>
            </div>
          )}
          {plafondPer>0&&tmi>=30&&(
